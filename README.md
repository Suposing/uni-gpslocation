...
`sup-gpslocation` 是一个基于 UTS 的前后台 GPS 定位插件仓库，当前同时包含：

- 插件源码：`uni_modules/sup-gpslocation`
- `uni-app x` 测试工程：用于验证前台定位、后台定位、最近位置获取、通知权限与系统设置跳转

当前版本信息：

- 插件版本：`0.0.3`
- HBuilderX：`^3.99`
- `uni-app` / `uni-app x`：`^4.07`

## 快速开始

1. 使用 HBuilderX 打开当前仓库根目录
2. 运行到 Android 或 iOS 真机
3. 进入首页定位测试台，按页面按钮逐项验证定位开关、后台权限、通知权限、最近位置与持续定位
4. 如需查看插件实现，直接检查 `uni_modules/sup-gpslocation`

说明：

- 当前仓库未配置 `npm` / `pnpm` 启动脚本
- 推荐通过 HBuilderX 真机运行验证
- Android 后台定位测试前，需同时确认系统定位、应用通知和后台定位权限均已开启

## 测试工程说明

当前测试工程名称为 `测试定位插件`，入口页面是一个定位测试台，主要覆盖以下能力：
...
- 定时轮询最近位置
- 开始 / 停止持续定位
- 切换 `gps`、`background`、`onlyOnce`、`coordType`、时间间隔、距离间隔和通知文案

测试页面位置：

- 页面入口：`pages/index/index.uvue`
- 应用入口：`App.uvue`、`main.uts`
- 应用配置：`manifest.json`、`pages.json`
